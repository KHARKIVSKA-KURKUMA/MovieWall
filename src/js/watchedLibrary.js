...
function renderLibrary(movies) {
  for (let i = 0; i < movies.length; i += 1) {
    fetchMovieForWatched(movies[i]).then(data => {
      refs.homeGalleryList.insertAdjacentHTML('beforeend', createLibraryMovieItem(data));
    });
  }
}
...
const posterSrc = poster_path
  ...
  const genresMovies = data.genres.map(element => element.name)
   const genresMovie = genresMovies.join(', ') ? genresMovies.slice(0, 2).join(', ') : 'Unknown genres';
